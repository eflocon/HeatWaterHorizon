...
//  digitalWrite(PinOutputFET, TRUE);  //LOWActive -> set LOW to MOSFET-GATE for Setup
  digitalWrite(PinOutputFET, FALSE);  //LOWActive -> set LOW to MOSFET-GATE for Setup
  Serial.begin(38400);
  sensors.begin();
  delay(1000);
  printLine("-");
...
void loop() {
  ...
  if(PumpON){
    digitalWrite(PinOutputLED, TRUE);
//    digitalWrite(PinOutputFET, FALSE);    //LOWActive -> set HIGH to MOSFET-GATE
    digitalWrite(PinOutputFET, TRUE);    //LOWActive -> set LOW to MOSFET-GATE
    Serial.println("Pump -> ON");    
  }
  else {
    digitalWrite(PinOutputLED, FALSE);
//    digitalWrite(PinOutputFET, TRUE);    //LOWActive -> set LOW to MOSFET-GATE
    digitalWrite(PinOutputFET, FALSE);    //LOWActive -> set LOW to MOSFET-GATE
